					...
					if ( $version ) {
						...
						if ( $plugin->iscore ) {
							...
							if ( $plgVersions ) foreach ( $plgVersions as $plgVersion ) {
								...
								$folderElement			=	str_replace( 'plug_', '', $plugin->folder );

								if ( ( $plgName == $plugin->name ) || ( $plgFolder == $plugin->folder ) || preg_match( '/' . preg_quote( $plgFolder ) . '$/i', $folderElement ) || preg_match( '/' . preg_quote( $folderElement ) . '$/i', $plgFolder ) ) {
