	...
	public function displayWeb()
	{
		...
		if ( Application::Input()->get( 'format', null, GetterInterface::STRING ) != 'raw' ) {
			...
			$js											=	"$.cbweb = function( refresh ) {"
														...
														.			"type: 'GET',"
														.			"data: { domain: '" . addslashes( $_CB_framework->getCfg( 'live_site' ) ) . "' },"
														.			"dataType: 'jsonp',"
														.			"crossDomain: true,"
														.			"cache: false"
		...
		foreach ( $packages as $typeId => $type ) {
			...
			$items[$typeValue]							=	$type->subTree( 'packages' );

			// Keep track of new and popular packages:
			foreach ( $items[$typeValue] as $packageId => $typePackage ) {
				/** @var ParamsInterface $typePackage */
				$packageId								=	( $typeId == '0' ? 'preset_' : 'package_' ) . $packageId;

				if ( $typePackage->get( 'new', false, GetterInterface::BOOLEAN ) ) {
					$new[$packageId]					=	$typePackage;
				} elseif ( $typePackage->get( 'popular', false, GetterInterface::BOOLEAN ) ) {
					$popular[$packageId]				=	$typePackage;
				}
			}

			$menu										.=	'<li class="cbWebType' . ( ! $items[$typeValue]->count() ? ' cbWebTypeEmpty' : null ) . '">'
			...
			foreach ( $type->subTree( 'subtypes' ) as $subTypeId => $subType ) {
				...
				$items[$subTypeValue]					=	$subType->subTree( 'packages' );

				// Keep track of new and popular subtype packages:
				foreach ( $items[$subTypeValue] as $packageId => $subTypePackage ) {
					/** @var ParamsInterface $subTypePackage */
					$packageId							=	( $typeId == '0' ? 'preset_' : 'package_' ) . $packageId;

					if ( $subTypePackage->get( 'new', false, GetterInterface::BOOLEAN ) ) {
						$new[$packageId]				=	$subTypePackage;
					} elseif ( $subTypePackage->get( 'popular', false, GetterInterface::BOOLEAN ) ) {
						$popular[$packageId]			=	$subTypePackage;
					}
				}
			}
		...
		if ( $news || $items['home'] ) {
			$return										.=			'<ul class="cbWebTypes nav nav-pills nav-stacked bg-muted">'
														.				'<li class="cbWebType active">'
		...
		foreach ( $items as $type => $typeItems ) {
			...
			foreach ( $typeItems as $itemId => $item ) {
				...
				if ( $item->get( 'version', null, GetterInterface::STRING ) ) {
					if ( $item->get( 'type', null, GetterInterface::INT ) == 8 ) {
						foreach ( $installedPlugins as $installedPlugin ) {
							$plgName					=	$item->get( 'name', GetterInterface::STRING );
							$plgFolder					=	trim( str_replace( $item->get( 'version', null, GetterInterface::STRING ), '', pathinfo( $item->get( 'file', null, GetterInterface::STRING ), PATHINFO_FILENAME ) ), '_ -' );
							$folderElement				=	str_replace( 'plug_', '', $installedPlugin->folder );

							if ( ( $plgName == $installedPlugin->name ) || ( $plgFolder == $installedPlugin->folder ) || preg_match( '/' . preg_quote( $plgFolder ) . '$/i', $folderElement ) || preg_match( '/' . preg_quote( $folderElement ) . '$/i', $plgFolder ) ) {
								...
								if ( strpos( $item->get( 'version', null, GetterInterface::STRING ), '+build' ) === false ) {
									// Stable doesn't store metadata in XML so we need to remove it before comparing:
									$latestCompare		=	preg_replace( '/\+.*/', '', $item->get( 'version', null, GetterInterface::STRING ) );
								} else {
									$latestCompare		=	str_replace( '+build.', '+', $item->get( 'version', null, GetterInterface::STRING ) );
								}
					...
					} elseif ( ( $item->get( 'type', null, GetterInterface::INT ) >= 1 ) && ( $item->get( 'type', null, GetterInterface::INT ) <= 7 ) ) {
						foreach ( $installedExtensions as $installedExtension ) {
							switch ( $item->get( 'type', null, GetterInterface::INT ) ) {
								case 1:
									$extType			=	'package';
									break;
								case 2:
							...
							$extName					=	$item->get( 'name', GetterInterface::STRING );
							$extFolder					=	trim( str_replace( $item->get( 'version', null, GetterInterface::STRING ), '', pathinfo( $item->get( 'file', null, GetterInterface::STRING ), PATHINFO_FILENAME ) ), '_ -' );

							if ( ( $extName == $installedExtension->name ) || ( $extFolder == $installedExtension->element ) || preg_match( '/' . preg_quote( $extFolder ) . '$/i', $installedExtension->element ) || preg_match( '/' . preg_quote( $installedExtension->element ) . '$/i', $extFolder ) ) {
				...
				if ( ! $item->get( 'conditioned', false, GetterInterface::BOOLEAN ) ) {
					if ( $item->get( 'type', null, GetterInterface::INT ) == 12 ) {
						...
						if ( $isLatest === 1 ) {
							$return						.=									'<a href="javascript: void(0);" class="btn btn-xs btn-default disabled">' . CBTxt::T( 'Already Installed' ) . '</a>';
						} elseif ( $isLatest === -1 ) {
		...
		if ( $cacheTime ) {
			$return										.=	'<div class="cbWebCached text-right text-small text-muted">'
