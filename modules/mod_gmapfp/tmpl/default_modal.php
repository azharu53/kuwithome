...
<?php foreach($gmapfps as $gmapfp) {?>
    <tr>
        <td>
            <a  class='gmafp_link_module' href="#" onclick="window.open('<?php echo JRoute::_(GMapFPHelperRoute::getArticleRoute($gmapfp->slug)) ?>', '', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=650,height=500')" title="<?php echo JText::_('gmapfp_more_infos') ?>">
                <?php
					if ($gmapfp->img!=null) 
						if ($params->get('Pro', 0)) {
